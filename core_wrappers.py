...
def pre_validation_checks(cfg, skip_target_validation=False):  # noqa pylint: disable=R0914,R0911
    ...
    # Conditionally add the keys for target validation
    if not skip_target_validation:
        required_keys["inputs"].extend(["TARGET_URL", "GCP_PROJECT_ID"])
...
def export_artifacts(cfg, resources_list): # noqa pylint: disable=R0914
    ...
    if os.environ.get("IGNORE_EXPORT") == "true":
        export_data = {}
        export_data["orgConfig"] = apigee_export.read_export_state(
            os.path.join(export_dir, "orgConfig")
        )  # noqa pylint: disable=C0301
        export_data["envConfig"] = apigee_export.read_export_state(
            os.path.join(export_dir, "envConfig")
        )  # noqa pylint: disable=C0301
    else:
...
def validate_artifacts(
    cfg, resources_list, export_data, skip_target_validation=False
):  # noqa pylint: disable=R0914,R0912,R0915
    ...
    if target_compare and skip_target_validation:
        logger.warning(
            "TARGET_COMPARE is set to true, but --skip-target-validation "
    ...
    if "all" in resources_list:
        ...
        target_resource_list = [
            r for r in resources_list if r in target_resources
        ]  # noqa pylint: disable=C0301
    ...
    apigee_validator = ApigeeValidator(
        target_url,
        gcp_project_id,
        gcp_token,
        gcp_env_type,
    ...
    for env, _ in export_data["envConfig"].items():
        ...
        if "all" in resources_list or "resourcefiles" in resources_list:
            report[env + SEPERATOR + "resourcefiles"] = (
                apigee_validator.validate_env_resourcefiles(env, resourcefiles)
            )  # noqa pylint: disable=C0301
        if "all" in resources_list or "flowhooks" in resources_list:
            report[env + SEPERATOR + "flowhooks"] = (
...
def visualize_artifacts(
    ...
    dg.nodes["ORG" + SEPERATOR + source_url][
        "title"
    ...
    for key, value in exportorg.items():  # noqa pylint: disable=R1702
        ...
        if key == "apiProducts":
            ...
            dg.nodes["ORG" + SEPERATOR + key.upper()][
                "title"
            ] = f'<a href={res_url} target="_blank">Org level {key}</a>'  # noqa
        elif key == "kvms":
        ...
        if len(value) > threshold:
            dg.add_edge(
                "More than " + str(threshold) + " " + key,
                "ORG" + SEPERATOR + key.upper(),
            )
            dg.nodes["More than " + str(threshold) + " " + key][
                "color"
            ] = "black"  # noqa
            dg.nodes["More than " + str(threshold) + " " + key]["size"] = 20
            dg.nodes["More than " + str(threshold) + " " + key]["title"] = (
                "Total - " + str(len(value)) + " " + key
            )  # noqa
            continue
            ...
            if key in ["apis", "sharedflows"]:
                if final_report.get(key):
                    if final_report.get(key, {}).get(name, True) is not True:
                        ...
                        count = 1
                        viols = ""
                        each_resource = final_report.get(key, {}).get(
                            name, [{"violations": []}]
                        )  # noqa pylint: disable=C0301
    ...
    dg.nodes["ORG" + SEPERATOR + "ENVs"][
        "title"
    ] = f'<a href={env_url} target="_blank">Environments</a>'  # noqa
        ...
        for resource, val in value.items():
            ...
            if resource == "resourcefiles":
                ...
                dg.nodes[env + SEPERATOR + resource.upper()][
                    "title"
                ] = f'<a href={res_url} target="_blank">Env {env} level {resource}</a>'  # noqa pylint: disable=C0301
            elif resource == "kvms":
            ...
            for name, _ in val.items():
                ...
                if resource in ["targetServers", "resourcefiles"]:
                    if (
                        ...
                        error_message = (
                            final_report[env + SEPERATOR + resource][name][0]
                            .get("error_msg", {})
                            .get("message", "")
                        )  # noqa pylint: disable=C0301
                        dg.nodes[env + SEPERATOR + name]["title"] = (
...
def get_topology(cfg):
    ...
    apigee_topology = ApigeeTopology(
        source_url, source_org, source_auth_token, source_auth_type, cfg
    )
